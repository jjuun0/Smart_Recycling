...
# 2021.07.15
- pytorch - Custom Dataset 을 구축하여 코드 처음부터 다시짜봄.
- init 에서는 파일 경로만 추가해주고, getitem 부분에서 이미지 파일을 연다
- init 에서 파일 이미지를 열면 메모리상에 모든 파일이 올라간다. 주의하도록.
- 또한 모델이 예측한 클래스를 바탕으로 서보모터 각도 제어를 함.
